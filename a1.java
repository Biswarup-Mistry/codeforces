...
class Array {
    public void run(){
        ...
        for (int i=0;i<t;i++){
            ...
            int maxA = Arrays.stream(arrayHolder[2*i]).max().getAsInt();
            int maxB = Arrays.stream(arrayHolder[2*i+1]).max().getAsInt();
            int size = Math.max(maxA, maxB) + 1;   // +1 for safety

            int[] D_a = new int[size];
            int[] D_b = new int[size];
